$(document).ready(function () {
    ...
    function AppViewModel() {
        ...
        self.isLoading = ko.observable(true); // lan dau dang nhap thi co show len trang web chat hay khong
        self.hasMoreMessages = ko.observable(true);
        self.heightMessage = ko.observable(0);
        ...
        self.showMessageMore = function () {
            self.countShowMessageMore(self.countShowMessageMore()+1);
            self.messageHistory();
        }
        ...
        self.messageHistory = function () {
            fetch('/api/Messages/Room/' + viewModel.joinedRoom() + '/' + self.countShowMessageMore())
                .then(response => response.json())
                .then(data => {
                    console.log(self.myName());
                    console.log(data)
                    if (data.length < self.countShowMessageMore() * 20) {
                        self.hasMoreMessages(false);
                    }
                    //console.log("hasMoreMessages", self.hasMoreMessages())
                    self.chatMessages.removeAll();
                    for (var i = 0; i < data.length; i++) {
                        var isMine = data[i].username == self.myName();
                    ...
                    if (self.countShowMessageMore() == 1) {
                        chat_body.animate({ scrollTop: $(".chat-body")[0].scrollHeight }, 1000);
                        self.heightMessage($(".chat-body")[0].scrollHeight);
                    }
                    else {
                        chat_body.animate({ scrollTop: $(".chat-body")[0].scrollHeight - self.heightMessage() }, 1);
                        console.log(self.heightMessage());
                        self.heightMessage($(".chat-body")[0].scrollHeight);
                    }
                });
        }
        ...
        self.roomUpdated = function (updatedRoom) {
            var room = ko.utils.arrayFirst(self.chatRfooms(), function (item) {
